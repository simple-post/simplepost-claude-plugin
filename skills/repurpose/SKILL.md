...
3. Invoke the `platform-copywriter` sub-agent once per selected platform, in parallel. Give each invocation the source idea, platform, account display name and ID, voice brief, and hard constraints. Tell it to apply the `platform-craft` principles.
